/**
 * Shared ESPN season-stats parsing logic, moved from
 * client/src/hooks/useESPNSeasonStats.ts so a server-side historical
 * backfill (server/historicalSeasonStatsBackfill.ts) can reuse the exact
 * same, already-proven parsing rather than a separate reimplementation.
 * Pure data transformation only -- no browser APIs (fetch, sessionStorage)
 * live here; those stay in the client hook, which now re-exports these.
 */
...
export function buildLabelMap(labels: string[]): Record<string, number> {
...
export type ESPNGameEvent = { eventId?: string; stats?: string[] };
...
export function calculateSeasonRow(year: number, pos: string, extracted: Partial<SeasonStatRow>): SeasonStatRow {
