...
import { finalizeWeeklyResultsFromTank, weeklyRecordDelta } from "./weeklyResultsFinalize";
...
const WRC_TEAM_ID_TO_OWNER: Record<string, string> = Object.fromEntries(
...
function nextDraftState(currentRound: number, currentPick: number, protectedSlots: Set<string> = new Set()) {
  let round = currentRound;
  let pick = currentPick;
  // Advance one slot at a time, skipping any cell a protection already
  // occupies, up to a full draft's worth of slots as a safety bound.
  for (let i = 0; i < WRC_DRAFT_TOTAL_ROUNDS * WRC_DRAFT_TOTAL_TEAMS; i++) {
    const nextPick = pick + 1 >= WRC_DRAFT_TOTAL_TEAMS ? 0 : pick + 1;
    const nextRound = pick + 1 >= WRC_DRAFT_TOTAL_TEAMS ? round + 1 : round;
    ...
    round = nextRound;
    pick = nextPick;
    if (!protectedSlots.has(`${round}-${pick}`)) {
      return { current_round: round, current_pick: pick, complete: false, paused: false, timer_seconds: WRC_DRAFT_TIMER_SECONDS };
    }
  }
  // Every remaining slot is protected (shouldn't happen) -- treat as complete.
  return { current_round: round, current_pick: pick, complete: true, paused: false, timer_seconds: WRC_DRAFT_TIMER_SECONDS };
}
...
export const appRouter = router({
  ...
  league: router({
    ...
    myFaabBids: teamProcedure
      ...
      .query(async ({ input, ctx }) => {
        const { data, error } = await supabaseAdmin
          ...
          .eq("season", input.season)
          .order("created_at", { ascending: false });
        if (error) throw new Error("Unable to load your FAAB bids.");
        return data ?? [];
      }),
    ...
    updateFaabBidAmount: teamProcedure
      .input(z.object({ bidId: z.string().min(1).max(128), bidAmount: z.number().int().min(0).max(10_000) }))
      .mutation(async ({ input, ctx }) => {
        const teamId = ctx.teamSession.teamId;
        ...
        const { error: updateError } = await supabaseAdmin
          .from("faab_bids")
          .update({ bid_amount: input.bidAmount })
          .eq("id", input.bidId)
          .eq("team_id", teamId)
          .eq("status", "pending");
    ...
    respondToTradeProposal: teamProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.action === "declined") {
          ...
          const { data: fromTeamPhone } = await supabaseAdmin.from("teams")
            .select("name, phone_number, sms_trade_notifications").eq("id", proposal.from_team_id).single();
          ...
          if (fromTeamPhone?.sms_trade_notifications && fromTeamPhone.phone_number) {
            await sendSms(
              ...
              `🏈 WRC Fantasy: ${decliningTeam?.name ?? "The other owner"} declined your trade proposal.`,
            ).catch(error => console.error("[respondToTradeProposal] SMS notification failed:", error));
          }
        ...
        if (fromTeamResponse.data.sms_trade_notifications && fromTeamResponse.data.phone_number) {
          await sendSms(
            ...
            `🏈 WRC Fantasy: ${toTeam.name} accepted your trade proposal! It's now final.`,
          ).catch(error => console.error("[respondToTradeProposal] SMS notification failed:", error));
        }
        ...
        if (input.action === "reset") {
          ...
          for (const { player_name } of picksBeingCleared ?? []) {
            const { error: unrosterError } = await supabaseAdmin.from("players")
              .update({ team_id: null, draft_round: null, draft_pick: null, acquisition: "FA" })
    ...
    commissionerEditDraftPick: commissionerProcedure
      .input(z.object({
        pickId: z.number().int(),
        newPlayerName: z.string().min(1).max(128),
        newPlayerPos: z.string().min(1).max(8),
        newPlayerNflTeam: z.string().min(1).max(8),
      }))
      .mutation(async ({ input }) => {
        const { data: existingPick, error: pickFetchError } = await supabaseAdmin
          .from("draft_picks").select("*").eq("id", input.pickId).single();
        ...
        if (draftPlayer.name.toLowerCase() === existingPick.player_name.toLowerCase()) {
          return { pick: existingPick, unchanged: true as const };
        }

        const { data: conflictingPick, error: conflictError } = await supabaseAdmin
          .from("draft_picks").select("id").ilike("player_name", draftPlayer.name).neq("id", input.pickId).maybeSingle();
        ...
        const { data: newRosteredPlayer, error: newRosteredError } = await supabaseAdmin
          .from("players").select("id, team_id").ilike("name", draftPlayer.name).maybeSingle();
        if (newRosteredError) throw new Error("Unable to verify player availability.");
        if (newRosteredPlayer?.team_id) {
          const { data: protection, error: protectionError } = await supabaseAdmin
            .from("protections").select("player_id").eq("player_id", newRosteredPlayer.id).maybeSingle();
          if (protectionError) throw new Error("Unable to verify protected player availability.");
        ...
        const { data: updatedPick, error: updateError } = await supabaseAdmin.from("draft_picks")
          .update({ player_name: draftPlayer.name, player_pos: draftPlayer.pos, player_nfl_team: draftPlayer.nflTeam })
          .eq("id", input.pickId)
          .select("id, round, pick, overall, team_name, owner, player_name, player_pos, player_nfl_team, picked_at")
          .single();
        ...
        return { pick: updatedPick, unchanged: false as const };
      }),
    commissionerFinalizeWeeklyResult: commissionerProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const addOutcome = (teamId: string, ownScore: number, opponentScore: number, median: number, divisionGame: boolean) => {
          ...
          if (h2hOutcome === "W") {
            row.h2h_wins += 1; if (divisionGame) row.div_wins += 1;
            row.streak = row.streak.startsWith("W") ? `W${Number(row.streak.slice(1)) + 1}` : "W1";
          } else if (h2hOutcome === "L") {
            row.h2h_losses += 1; if (divisionGame) row.div_losses += 1;
          ...
          const beatMedian = ownScore > median;
