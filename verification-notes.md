...
The completed 2026 NFL Draft listing identifies fantasy-eligible rookies such as Fernando Mendoza (QB, LVR), Jeremiyah Love (RB, ARI), Carnell Tate (WR, TEN), Jordyn Tyson (WR, NO), Ty Simpson (QB, LAR), Kenyon Sadiq (TE, NYJ), Makai Lemon (WR, PHI), KC Concepcion (WR, CLE), Jadarian Price (RB, SEA), De'Zhaun Stribling (WR, SF), Denzel Boston (WR, CLE), and Eli Stowers (TE, PHI). The static WRC Draft Board module contains none of these verified 2026 rookie names. The Draft Board itself was opened without making a pick and showed the draft grid and sign-in prompt; the full selectable pool is intentionally owner-gated through the draft queue.

## Draft Players Current-Season ADP and Retired-Player Correction — 2026-08-18

The prior Draft Players board combined two Tank01 paths: the raw `getNFLPlayerList` candidate feed and legacy embedded ADP values from the original static player pool. That candidate feed still listed Amari Cooper as a Las Vegas wide receiver, while ESPN’s current 2026 Raiders roster feed did not include him. The corrected generator now accepts a Tank01 QB/RB/WR/TE/K candidate only when the player and position also appear on an ESPN current NFL team roster. The two previously identified official 2026 drafted tight ends remain explicit rookie overrides because an official roster feed may not yet list them.

PPR ADP now comes only from Tank01 `getNFLADP?adpType=PPR` when its payload explicitly identifies a 2026 date. The audited live response reported `adpDate: 20260818`; browser cache moved to `v4` and rejects older or undated payloads. The regenerated universe contains 1,001 active-roster or official-rookie entries across all 32 teams, excludes 508 unverified Tank01 candidates including Amari Cooper, and labels the browser board `Tank01 PPR ADP · 2026-08-18`. Browser verification showed the corrected 2026 source label and no Amari Cooper record. Full validation passed: 39 test files, 102 tests, TypeScript, and production build.

## Compact Draft Players Queue Star and Pinned Identity — 2026-08-18

The Draft Players table now removes the large right-edge Add to Queue control. A compact `Que` column appears immediately before `POS`, where an outlined star adds the player to the signed-in owner’s private queue and a filled gold star removes the queued player. The Player header and player identity cell are pinned at the left edge of the horizontally scrollable table. Desktop verification showed the compact Que/POS/NFL/ADP layout with no large action button. A 390px mobile capture confirmed the Que header and each star remain visible beside Player; the table minimum width was reduced to 600px to retain those controls while positional, team, and ADP columns remain scrollable to the right. Focused queue-toggle tests and TypeScript validation pass.

## Draft Players Free Agents Identity, Headshots, and Leading Stats — 2026-08-18

Draft Players now mirrors the compact Free Agents identity treatment: the pinned Player cell contains an ESPN headshot resolved from the generated source-player ID, a circular initials fallback for unresolved players, and an NFL team-logo treatment for D/ST. It links to the Player Card and places position plus NFL team beneath the player name. The leading table order is now `#`, `Player`, `Que`, `Bye`, `FPTS`, `FP/G`, and `ADP`; Que remains adjacent to Player and both stay pinned on the left during horizontal scrolling.

The FPTS and FP/G cells use the same completed 2025 position-aware season-stat snapshot as Free Agents. The board deliberately suppresses individual Tank01 fallback lookups, which can receive an HTTP 451 in this environment, so unsupported/no-prior-NFL-season rows render a clear `—` rather than indefinite loading. The top current ADP rows are 2026 rookies and therefore correctly show no completed NFL 2025 FPTS/FP/G. Desktop and 390px mobile captures verified the requested headers, headshots, pinned Que control, and mobile identity presentation. Focused formatting and queue tests plus TypeScript validation passed.

## Jacksonville 2026 Bye-Week Reconciliation — 2026-08-18

The generated Draft Players universe inherited a stale Week 1 Jacksonville bye from its prior static source. The official Jaguars 2026 schedule release states that Jacksonville has its bye in **Week 7** after the two London games, and the official NFL schedule independently lists `JAGUARS' BYE WEEK` in Week 7. The reproducible universe generator now applies JAC = Week 7 after reading the legacy all-team map; regeneration confirmed every current JAC player record, including Trevor Lawrence, Carter Bradley, and Joey Aguilar, now carries `bye: 7`. Sources: https://www.jaguars.com/news/official-jaguars-2026-schedule-released and https://www.nfl.com/schedules/2026/by-team/jacksonville-jaguars

## Draft Players Sort, Completed Stats, and Sticky Header Corrections — 2026-08-18

Every displayed Draft Players header is now actionable for ascending and descending sorting. Numeric sorting covers Bye, FPTS, FP/G, and ADP while keeping unavailable values at the end in either direction; queue, Player, and position sorting are also covered. Regression tests confirm the verified Week 7 JAC assignment for every current Jaguars record and the 2025 WRC 1.5-PPR TE calculation used by the completed-season display.

The previous completed-offense snapshot covered only 161 records, so many legitimate 2025 veterans appeared as unavailable. A reconciled completed-2025 play-by-play dataset expands that snapshot to 505 offensive player lines and is served from the durable shared cache. The cache payload column was safely expanded from `TEXT` to `MEDIUMTEXT` after the 130 KB reconciled snapshot exceeded the former capacity. Browser caching of the endpoint is disabled so a stale response cannot mask a correction; versioned local stat caching remains in place. Fresh browser validation showed Kenny Gainwell at 220.3 FPTS / 13.0 FP/G and Chig Okonkwo at 152.0 FPTS / 9.5 FP/G, while 2026 rookies correctly remain unavailable for completed 2025 NFL statistics.

The mobile table now pins the `#`, `Player`, and `Que` headers at exactly the same horizontal offsets as their body columns. This keeps the Player header aligned over the player identity card while the owner scrolls toward Bye, FPTS, FP/G, and ADP.

## Owner Passkeys, Persistent Sessions, and Link Preview — 2026-08-20

The secure owner-session lifetime is now 30 days while retaining the existing httpOnly, secure, signed-cookie behavior and explicit sign-out path. The new passkey flow uses the WRC public domains as its relying-party origins, requires user verification, creates short-lived single-use challenges, persists only public-key credential material, and retains the PIN path for enrollment, recovery, and unsupported devices. The manually applied Supabase migration was verified through the service role: both `team_passkeys` and `team_passkey_challenges` exist and initially contain zero rows.

The mobile development login page was checked at 390×844. The existing PIN login card, selector, and sign-in action remain intact. Face ID controls are intentionally suppressed in the development preview because passkeys are restricted to `wrcfantasyfootball.com` and `www.wrcfantasyfootball.com`; on those public domains, supported iPhone and device-lock authenticators expose the passkey sign-in control. Settings provides passkey enrollment, status, and removal controls on supported public devices. The shared page head now contains Open Graph and compatible card metadata that points to the WRC 512×512 league logo for new Messages and social link previews.

Immediately after checkpoint `583b23f8`, the custom domain returned the prior public document without the new Open Graph tags, even though its visible login shell was healthy. This is the expected short propagation interval after an automatic publication; metadata will be rechecked against the public page once the current application shell reaches the edge.
