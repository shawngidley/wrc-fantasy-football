...
-- Current NFL team (and derived bye week) per player, keyed by the same
-- sourcePlayerId (Tank01/ESPN player id) used throughout
-- currentDraftPlayerUniverse2026.ts. That file is a point-in-time static
-- snapshot regenerated by hand; this table is the daily-refreshed live
-- override layer on top of it, so a trade/signing/release is reflected
-- across the app without needing a manual pool regeneration or redeploy.
create table if not exists nfl_team_assignments (
  source_player_id varchar(32) primary key,
  nfl_team varchar(8) not null,
  bye_week integer,
  refreshed_at timestamptz not null default now()
);
...
alter table nfl_team_assignments enable row level security;
