    ...
    <main style={{ maxWidth: 920, margin: "0 auto", padding: "1rem 1rem 3rem" }}>
      ...
      <section className="wrc-card" style={{ overflow: "hidden" }}><div className="wrc-card-gold-stripe" /><div style={{ padding: "1.25rem" }}>
        ...
        <div aria-live="polite" style={{ position:"relative", minHeight:330, marginTop:"1rem", borderRadius:20, overflow:"hidden", background:"radial-gradient(circle at 50% 0%, oklch(0.37 0.11 150 / .88), oklch(0.14 0.04 150))", border:"2px solid oklch(0.72 0.15 85 / .72)", boxShadow:"inset 0 0 50px rgb(0 0 0 / .45)" }}>
          ...
          {remainingOwners.map((owner,index) => <div className="lottery-ball" key={owner} style={{ position:"absolute", left:`${13 + ((index * 29) % 66)}%`, top:`${26 + ((index * 17) % 45)}%`, animation:`lotteryFloat ${2.2 + index*.22}s ease-in-out ${index*.15}s infinite`, borderRadius:999, padding:".55rem .75rem", background:"oklch(0.93 0.02 150)", border:"2px solid oklch(0.78 0.15 85)", color:"oklch(0.2 0.07 150)", boxShadow:"0 5px 14px rgb(0 0 0 / .34)", fontFamily:"Barlow Condensed, sans-serif", fontWeight:800, fontSize:".82rem", zIndex:2 }}>{owner}</div>)}
