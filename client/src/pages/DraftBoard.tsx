...
function DraftCountdownBanner() {
  ...
  const lblStyle: React.CSSProperties = {
    fontFamily: "Barlow Condensed, sans-serif", fontWeight: 400, fontSize: "0.65rem",
    letterSpacing: "0.1em", textTransform: "uppercase",
    color: "rgba(255,255,255,0.55)", marginTop: 2,
  };
  const sepStyle: React.CSSProperties = {
    fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, fontSize: "1.6rem",
        ...
        {draftStarted && !draftComplete && (
          <div style={{
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, fontSize: "2rem", color: timerColor, lineHeight: 1 }}>
                {timerStr}
              </div>
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              ...
              {isCommissioner && (
                <>
                  <button onClick={() => setIsPaused(p => !p)} style={{ background: "rgba(255,255,255,0.12)", color: "white", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "0.5rem 0.75rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.3rem", fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.78rem" }}>
        ...
        <div style={{ overflowX: "auto", marginBottom: "1.5rem" }}>
          <div style={{ minWidth: TOTAL_TEAMS * 120 + 60 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: `60px repeat(${TOTAL_TEAMS}, 1fr)`, gap: 2, marginBottom: 2 }}>
              ...
              {TEAMS.map((team, i) => (
                <div key={i} style={{ background: "oklch(0.22 0.08 150)", padding: "0.4rem 0.25rem", fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.65rem", fontWeight: 600, color: "white", textAlign: "center", letterSpacing: "0.02em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {team.replace("Team ", "")}
                </div>
              ))}
            </div>
                ...
                {Array.from({ length: TOTAL_TEAMS }, (_, p) => {
                  ...
                  return (
                    <div key={p} style={{
                      ...
                      animation: isCurrent ? "gold-pulse 1.4s ease-in-out infinite" : pick ? "fadeInUp 0.25s ease both" : "none",
                    }}>
                      {pick ? (
                        <>
                          <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.68rem", fontWeight: 700, color: "oklch(0.22 0.08 150)", textAlign: "center", lineHeight: 1.2 }}>{pick.player}</div>
                        ...
                        <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.65rem", fontWeight: 700, color: "oklch(0.15 0.02 150)", textAlign: "center" }}>ON CLOCK</div>
                      ) : (
                        <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.6rem", color: "rgba(255,255,255,0.25)", textAlign: "center" }}>{r + 1}.{p + 1}</div>
                      )}
                    </div>
                  );
                })}
                  ...
                  {tradedPicks.map((pick, i) => (
                    <tr key={pick.overall} style={{ background: i%2===0?"white":"oklch(0.97 0.005 150)", borderBottom: "1px solid oklch(0.93 0.01 150)" }}>
                      ...
                      <td style={{ padding: "0.45rem 0.75rem" }}>
                        <span style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, color: OWNER_COLORS[pick.owner]??"oklch(0.35 0.06 150)", fontSize: "0.78rem" }}>{pick.owner}</span>
                      </td>
                      <td style={{ padding: "0.45rem 0.75rem" }}>
                        <span style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, color: OWNER_COLORS[pick.originalOwner!]??"oklch(0.5 0.04 150)", fontSize: "0.78rem" }}>{pick.originalOwner}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div style={{ background: "white", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 600, maxHeight: "80vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {filteredPlayers.map(player => (
                <div key={player.id} onClick={() => handlePickPlayer(player)} className="wrc-row-hover" style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem 1.25rem", borderBottom: "1px solid oklch(0.95 0.003 150)", cursor: "pointer" }}
                >
                  <span style={{ width: 36, textAlign: "center", fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.7rem", fontWeight: 700, color: "white", background: POS_COLORS[player.pos] || "#64748b", borderRadius: 4, padding: "2px 0" }}>{player.pos}</span>
                  <div style={{ flex: 1 }}>
                  ...
                  <span style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: "0.78rem", fontWeight: 600, color: "oklch(0.28 0.09 150)" }}>Draft</span>
                </div>
              ))}
              {filteredPlayers.length === 0 && (
                <div style={{ padding: "2.5rem 1.25rem", textAlign: "center", color: "oklch(0.6 0.04 150)" }}>
                  <Search size={28} style={{ margin: "0 auto 0.5rem", opacity: 0.3 }} />
                  <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, letterSpacing: "0.04em", fontSize: "0.95rem" }}>No players found</div>
