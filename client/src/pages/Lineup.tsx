...
function isPlayerLocked(nflTeam: string, matchupMap: NFLMatchupMap): boolean {
  return hasTeamGameStarted(nflTeam, matchupMap);
}
...
function LineupIdentity({ player, meta, injuryDesignation }: { player: Player; meta?: { age?: string; headshot?: string }; injuryDesignation?: string }) {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "var(--lineup-identity-gap)", minWidth: 0 }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4, minWidth: 0 }}>
          <div aria-label={player.name} style={{ color: "oklch(0.2 0.09 250)", fontWeight: 800, fontSize: "var(--lineup-player-name-size)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", minWidth: 0 }}><span className="lineup-player-name-full" aria-hidden="true">{player.name}</span><span className="lineup-player-name-compact" aria-hidden="true">{mobileLineupName(player)}</span></div>
          <InjuryTag designation={injuryDesignation} size="xs" />
        </div>
...
export function LineupRosterTable({
  ...
  const decisionHeaders = ["BYE", "OPP", "GAME"];
        ...
        <table style={{ minWidth, width: "max-content", borderCollapse: "separate", borderSpacing: 0, background: "white" }}>
          <thead>
            <tr>
              ...
              <th colSpan={3} style={groupStyle}>WEEKLY DECISION</th><th colSpan={3} style={groupStyle}>FANTASY</th>
          ...
          <tbody>
            {players.map((player, index) => {
              ...
              return <Fragment key={player.id}><tr style={{ background: rowBg }}>
                ...
                <td onClick={() => onPlayerClick(player)} style={{ ...tdStyle, position: "sticky", left: slotWidth, zIndex: 2, minWidth: playerWidth, maxWidth: playerWidth, textAlign: "left", cursor: "pointer", background: rowBg, boxShadow: benchPinnedOutline }}><LineupIdentity player={player} meta={meta} injuryDesignation={injury} /></td>
