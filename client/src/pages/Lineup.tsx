...
export default function Lineup() {
  ...
  const { statMap: lineupStatMap, playerMetaMap: lineupMetaMap, zeroGpDebug } = useNFLSeasonStats(
    lineupSeasonPlayers,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "0.75rem", marginBottom: "1.25rem" }}>
          ...
          {zeroGpDebug.length > 0 && (
            <div style={{ fontSize: "0.6rem", fontFamily: "monospace", background: "oklch(0.95 0.02 85)", border: "1px solid oklch(0.8 0.05 85)", padding: "0.4rem", wordBreak: "break-all", color: "black", margin: "0.5rem 0" }}>
              {zeroGpDebug.map((line, i) => <div key={i}>DEBUG zero-gp: {line}</div>)}
            </div>
          )}
