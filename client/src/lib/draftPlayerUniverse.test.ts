...
describe("2026 WRC Draft player universe", () => {
  it("covers all NFL teams and the complete current Tank01 kicker inventory", () => {
    expect(CURRENT_DRAFT_PLAYER_UNIVERSE_2026).toHaveLength(1509);
    expect(new Set(CURRENT_DRAFT_PLAYER_UNIVERSE_2026.map(player => player.nflTeam)).size).toBe(32);
    expect(CURRENT_DRAFT_PLAYER_UNIVERSE_2026.filter(player => player.pos === "K")).toHaveLength(71);
  });
